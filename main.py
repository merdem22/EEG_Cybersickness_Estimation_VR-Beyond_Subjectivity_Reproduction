...
if __name__ == '__main__':
    ...
    parser.add_argument('--acc-threshold', type=float, default=0.10,
                    help='Binary sickness threshold (same scale as targets).')
    parser.add_argument('--acc-neighborhood', type=int, default=0,
                    help='Neighborhood radius (in windows); 0=none, 1=±1 window.')
    ...
    net = MyTrainerModel(
        ...
        train_joy_mean=float(np.mean(train_dataset.train_mean)),
        acc_threshold=params.acc_threshold,
        acc_neighborhood=params.acc_neighborhood,
    )
    ...
    train_callbacks = [AverageScoreLogger('MAE','MSE','Accuracy', level=20)]
    eval_callbacks  = [AverageScoreLogger('MAE','MSE','Accuracy', level=20)]
