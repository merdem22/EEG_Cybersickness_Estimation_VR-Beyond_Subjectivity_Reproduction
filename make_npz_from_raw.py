# make_npz_from_raw.py (fixed: return count + tmp filename)
...
def _build_one(args_tuple):
    (
        ...
        bandwidth, psd_raw_source, compress, joy_format, skip_existing, stage_dir
    ) = args_tuple

    try:
        pid_hint = participant_path.name

        # final output path (we’ll update pid after extraction if available)
        ...
        # use participant_id if provided by extractor
        ...
        # 2) PSDs
        # --psd-raw time  => store time-aligned PSD as psd_raw (N, C, T), no extra raw spectrum
        # --psd-raw raw   => also return the raw multitaper spectrum (N, C, F)
        need_raw = (psd_raw_source == "raw")
        if need_raw:
            psd_time, freqs, psd_raw = compute_psd_batch(
                windows, sfreq=sfreq, fmin=fmin, fmax=fmax,
                bandwidth=bandwidth, return_raw=True
            )  # psd_time: (N, C, T), psd_raw: (N, C, F)
        else:
            psd_time, freqs = compute_psd_batch(
                windows, sfreq=sfreq, fmin=fmin, fmax=fmax,
                bandwidth=bandwidth, return_raw=False
            )  # psd_time: (N, C, T)
            psd_raw = None  # not used in this mode

        # 3) shapes & dtype once
        windows   = np.asarray(windows,  dtype=np.float32, order="C")   # (N, C, T) from extractor
        psd_time  = np.asarray(psd_time, dtype=np.float32, order="C")   # (N, C, T)
        if need_raw and psd_raw is not None:
            psd_raw = np.asarray(psd_raw, dtype=np.float32, order="C")  # (N, C, F)

        # 4) store tensors
        # Save EEG as (N, T, C), PSD (time-aligned) as (N, T, C)
        eeg_for_file = np.transpose(windows,  (0, 2, 1))  # (N, T, C)
        psd_for_file = np.transpose(psd_time, (0, 2, 1))  # (N, T, C)

        # psd_raw_for_file: either the time-aligned PSD (N, C, T) or the raw spectrum (N, C, F)
        if psd_raw_source == "time":
            psd_raw_for_file = psd_time                     # (N, C, T)
        else:
        ...
        if joy_format == "legacy":
            # list-of-1 arrays (compatible with some older loaders)
            joy = [np.array([float(x)], dtype=np.float32) for x in cs_labels]
        else:
            joy = np.asarray(cs_labels, dtype=np.float32).reshape(N, 1)

        dataset = dict(
            eeg=eeg_for_file,                               # (N, T, C)
            psd=psd_for_file,                               # (N, T, C)
            psd_raw=np.asarray(psd_raw_for_file, dtype=np.float32),  # (N, C, T) or (N, C, F)
            joy=joy,
            tf=np.empty((0,), dtype=np.float32),            # placeholders to avoid object dtypes
            pth=np.empty((0,), dtype="U1"),
        )

        # 6) stage locally then move
        tmp_dir = Path(stage_dir) if stage_dir else final_out.parent
        ensure_dir(tmp_dir)
        # IMPORTANT: end temp name with .npz so np.savez won't append another .npz
        tmp_path = tmp_dir / (out_name + ".tmp.npz")

        if compress == "zip":
            np.savez_compressed(tmp_path, dataset=dataset)
        else:
            np.savez(tmp_path, dataset=dataset)  # writes exactly to tmp_path

        ensure_dir(final_out.parent)
        shutil.move(str(tmp_path), str(final_out))
        shape_info = np.asarray(psd_raw_for_file).shape
        return f"[ok] {final_out} → psd_raw.shape={shape_info}"
...
def main():
    ...
    ap.add_argument("--psd-raw",   type=str, default="time", choices=["time","raw"],
                    help="Store psd_raw as 'time' (N,C,T) or 'raw' (N,C,F).")
    ...
    ap.add_argument("--compress", choices=["zip","none"], default="zip",
                    help="'zip' smaller but slower; 'none' faster but larger.")
    ap.add_argument("--joy-format", choices=["legacy","array"], default="legacy",
                    help="Use 'legacy' for list-of-1 arrays; 'array' for an (N,1) float32 array.")
    ap.add_argument("--stage-dir", type=str, default=None,
                    help="Optional local staging dir for fast writes before moving to Drive.")
    args = ap.parse_args()
    ...
    for p in participants:
        jobs.append((
            p, out_dir, 1, args.task, args.sfreq, args.fmin, args.fmax,
